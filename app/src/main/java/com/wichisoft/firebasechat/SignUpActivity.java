...
public class SignUpActivity extends AppCompatActivity {
                        ...
                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                ...
                                if (task.isSuccessful()) {
                                    ...
                                    /*** creo usuario en firebase y lo redirecciono si OK*/
                                    createUserInDb(newUser.getUid(), newUser.getDisplayName(), newUser.getEmail(), 0.0, 0.0);
                                }else{
                                    //error
                                    Toast.makeText(SignUpActivity.this, "Error " + task.getException().getLocalizedMessage(), Toast.LENGTH_SHORT).show();
                                }
    ...
    private void createUserInDb(final String userId, final String displayName, final String email, final Double latitude, final Double longitude){
        mUsersDBref = FirebaseDatabase.getInstance().getReference().child("Users");
        final User user = new User(userId, displayName, email, 0.0, 0.0);
        mUsersDBref.child(userId).setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(!task.isSuccessful()){
                    ...
                    mUsersDBref.child(userId).updateChildren(result);
